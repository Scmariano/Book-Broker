...
public class BookController {
	...
	public String bookmarket(HttpSession session, Model model) {
		...
		if(userId == null) {
			...
			model.addAttribute("books", bookServ.unborrowedBooks(userServ.findById(userId)));
			model.addAttribute("myBooks", bookServ.borrowedBooks(userServ.findById(userId)));
	...
	public String borrow(@PathVariable("bookId")Long bookId, HttpSession session) {
		...
		if(userId == null) {
			...
			bookServ.addBorrower(bookServ.findBookId(bookId), userServ.findById(userId));
			return "redirect:/bookmarket";
		}
	}
	
	@RequestMapping("/bookmarket/{bookId}/return")
	public String returnBook (@PathVariable("bookId") Long bookId, HttpSession session) {
		Long userId = (Long) session.getAttribute("userId");
		if(userId == null) {
			return "redirect:/logout";
		}else {
			bookServ.removeBorrower(bookServ.findBookId(bookId));
			return "redirect:/bookmarket";
		}
	}
